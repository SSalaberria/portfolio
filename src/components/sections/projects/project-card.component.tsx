...
export const ProjectCard = forwardRef<HTMLDivElement, ProjectCardProps>(function ProjectCard(
  { title, description, website, repository, tags }: ProjectCardProps,
  ref,
) {
  return (
    <div ref={ref} className="flex">
      <div className="group relative flex w-[19rem] cursor-pointer flex-col gap-6 rounded-md bg-background-light-secondary p-8 transition-transform duration-300 will-change-contents hover:-translate-y-2 dark:bg-background-dark-secondary">
        ...
        <div className="flex flex-col gap-2">
          <p className=" text-l font-bold transition-colors duration-200 group-hover:text-primary-500">
            <a
        ...
        <div className="mt-auto flex flex-wrap gap-1">
          {tags.map((tag) => (
            <span
              key={tag}
              className=" rounded-full bg-[#eaeae9] px-2 py-1 text-s dark:bg-[#393b3e]"
            >
              {tag}
            </span>
          ))}
